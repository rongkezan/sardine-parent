...
import sardine.aes.utils.SardineException;
  ...
  public static String execute(Request request) {
    ...
    try {
      ...
      if (response.code() != 200) {
        throw new SardineException(SardineException.EXEC_ERROR, "[Execute] Response Status Error : " + response.code() + " message:" + response.message());
      }
      if (response.body() != null) {
        str = response.body().string();
        response.close();
      } else {
        throw new SardineException(SardineException.ENV_ERROR, "[Execute] Cannot get the response from server");
      }
      log.debug("[Response]{}", str);
      return str;
    } catch (IOException e) {
      e.printStackTrace();
      throw new SardineException(SardineException.RUNTIME_ERROR, "[Execute] Cannot get the response from server");
    }
